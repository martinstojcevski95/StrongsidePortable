    ...
    public class LocomotionSystemTarget : ILocomotionSystem
    {
        ...
        public override CharacterLocomotion.LOCOMOTION_SYSTEM Update()
        {
            ...
            if (this.usingNavmesh)
            {
                ...
                if ((!agent.hasPath || agent.pathStatus != NavMeshPathStatus.PathComplete) && pathPoints.Count == 0)
                {
                    ...
                    if (!agent.hasPath && distance < STOP_THRESHOLD && pathPoints.Count == 0)
                    {
                        this.Stopping();
                    }
                ...
                if (!hasStopped)
                {
                    if (remainingDistance <= this.stopThreshold && pathPoints.Count == 0)
                    {
                        agent.updateRotation = true;
                        this.Stopping();
                    }
                    else if (remainingDistance <= this.stopThreshold + SLOW_THRESHOLD && pathPoints.Count == 0)
                    {
                        this.Slowing(remainingDistance);
                    }
                    else if (remainingDistance <= this.stopThreshold + SLOW_THRESHOLD && pathPoints.Count != 0)
                    {
                        this.characterLocomotion.navmeshAgent.SetDestination(pathPoints.Dequeue());
                    }
                    else
                    {
                        this.Moving();
        ...
        public void SetAStarTarget(Vector3 position, TargetRotation rotation,
            float stopThreshold, UnityAction callback = null)
        {
            hasStopped = false;
            this.move = true;
        ...
        public void SetNavMeshTarget(Vector3[] positions, TargetRotation rotation,
            float stopThreshold, UnityAction callback = null)
        {
            this.move = true;
            hasStopped = false;
            ...
            if (this.characterLocomotion.canUseNavigationMesh)
            {
                ...
                {
                    pathPoints.Enqueue(position);
                }

                if (pathPoints.Count > 0)
                {
                    Debug.DrawLine(positions[positions.Length - 1], positions[positions.Length - 1] + Vector3.up,
                        Color.green, 5f);
                    ...
                    this.characterLocomotion.navmeshAgent.updatePosition = true;
                    this.characterLocomotion.navmeshAgent.updateUpAxis = true;
                    if (this.characterLocomotion.navmeshAgent.hasPath)
                        this.characterLocomotion.navmeshAgent.path = null;
                    this.characterLocomotion.navmeshAgent.isStopped = false;
                    this.characterLocomotion.navmeshAgent.SetDestination(pathPoints.Dequeue());
                }
            }
